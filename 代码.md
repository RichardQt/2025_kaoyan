...
```c
int seat = 20; //椅子数量
semaphore mutex = 1;   // 互斥信号量，控制对seat的访问
semaphore server = 0;  // 理发师信号量，顾客通过它唤醒理发师
semaphore customer = 0; // 顾客信号量，顾客等待理发师理发

void customer() {
    p(mutex);  // 进入临界区，检查椅子是否有空
    if (seat == 0) {  // 如果没有空椅子
        v(mutex);  // 退出临界区，顾客离开
        return;
    } else {
